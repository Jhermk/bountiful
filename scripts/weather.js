...
// get current weather
fetch(`https://api.openweathermap.org/data/2.5/weather?id=${CITY_ID_1}&appid=${API_KEY_1}&units=metric`)
  .then(response => response.json())
  .then(data => {
    // update HTML elements with current weather data
    document.getElementById('current-temp').innerText = `Temperature: ${data.main.temp.toFixed(0)} °C`;
    document.getElementById('current-condition').innerText = `Condition: ${data.weather[0].description}`;
  ...
  fetch(`https://api.openweathermap.org/data/2.5/forecast?id=${CITY_ID}&cnt=${COUNT}&units=imperial&appid=${API_KEY}`)
    .then(response => response.json())
    .then(data => {
      ...
      for (let i = 0; i < forecastList.length; i++) {
        ...
        if (currentDate === null || date.getDate() !== currentDate) {
          ...
          forecastList[i].querySelector('.forecast-date').innerText = `${dayOfWeek}, ${month} ${dayOfMonth}`;
        } else {
        ...
        forecastList[i].querySelector('.forecast-icon').innerHTML = `<img src="https://openweathermap.org/img/w/${iconCode}.png" alt="${forecast.weather[0].description}" />`;
        forecastList[i].querySelector('.forecast-temp').innerText = `${temperature}°F`;
      }
